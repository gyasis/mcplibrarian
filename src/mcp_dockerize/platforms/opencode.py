"""opencode platform config integration."""
...
class OpenCodePlatform(AbstractPlatform):
    """Writes MCP server entries to opencode's ``config.json``.
    ...
    def add_server(self, entry: RegistryEntry, compose_path: Path) -> None:
        """Add MCP server entry to opencode config.
        ...
        Creates ``~/.config/opencode/config.json`` with
        ``{"mcpServers": {}}`` if the file does not exist.
        """
    ...
    def remove_server(self, name: str) -> None:
        """Remove MCP server entry from opencode config.
